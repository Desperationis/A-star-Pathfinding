...
void Pathfinder::update(std::vector<std::vector<int>>& grid) {
	...
	// (if all two lowest scores are the same, compute the one most recently added
...
void Pathfinder::calculate(int x, int y, int xoffset, int yoffset, std::vector<std::vector<int>>& grid) {
	...
	//set the node spot to path (yellow)

	printf("Heuristic: %d  G: %d  Score: %d\n\n",
		openList[(x + xoffset) + ((y + yoffset) * grid[0].size())].h,
		openList[(x + xoffset) + ((y + yoffset) * grid[0].size())].g,
		openList[(x + xoffset) + ((y + yoffset) * grid[0].size())].score);
	//print for debug
